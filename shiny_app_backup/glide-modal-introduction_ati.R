glide_modal_introduction_ati <- function(input, output, session) {
  
observe({
  if (input$toggleSidebar_ati) {
    shinyjs::show(id = "Sidebar_ati")
  } else {
    shinyjs::hide(id = "Sidebar_ati")
  }
})
  ...
  glide(
    ...
    screen(
      tags$div(style = "text-align:center;", 
               ...
               actionButton(inputId = "goButton_obs_ati", 
...
observe({
  updateSliderInput(session, "deslizador_ati", value = input$deslizador_obs_ati)
})

observe({
  updateMaterialSwitch(session, "race1_ati", value = input$race1_obs_ati)
})

observe({
  updateMaterialSwitch(session, "race2_ati", value = input$race2_obs_ati)
})

observe({
  updateMaterialSwitch(session, "race3_ati", value = input$race3_obs_ati)
})

observe({
  updateMaterialSwitch(session, "race4_ati", value = input$race4_obs_ati)
})

observe({
  updateMaterialSwitch(session, "us_ati", value = input$us_obs_ati)
})

observe({
  updateMaterialSwitch(session, "foreign_ati", value = input$foreign_obs_ati)
})

observe({
  updateSliderTextInput(session, "cohort_ati",choices = c(1940:1998),
                        selected = input$cohort_obs_ati)
})

observe({
  updateActionButton(session, "goButton_obs_ati")
})
